...
.PHONY: build-api-dev
build-api-dev: ## Build API development image (carcel/wishlist/dev:php).
	@docker-compose build --pull php

.PHONY: build-client-dev
build-client-dev: ## Build client development image (carcel/wishlist/dev:node).
...
build-api-prod: ## Build API production images (carcel/wishlist/api:fpm and carcel/wishlist/api:nginx).
	@docker-compose build --parallel --pull api fpm

.PHONY: build-client-prod
build-client-prod: ## Build client production image (carcel/wishlist/client:latest).
...
.PHONY: fix-api-code
fix-api-code: ## Attempt to fix the violations detected by PHP CS Fixer.
